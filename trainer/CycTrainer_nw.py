...
import numpy as np
import cv2
from .prostrate import SliceData
from . import aux_dataset
...
class Cyc_Trainer_nw():
    def __init__(self, config):
        ...
        #contex discriminator 
        self.netD_B = ContextDiscriminator_attention().cuda()   #changed
        
        self.optimizer_D_B = torch.optim.Adam(self.netD_B.parameters(), lr=config['lr'], betas=(0.5, 0.999))
        self.aux_data = aux_dataset.AuxAttnDataset(7000,7000, config['gpu_ids'], mask_size = 160)
        ...
        #changed dataloader
        data = SliceData(config['dataroot'], mode ='train')
        self.dataloader = DataLoader(data, batch_size= config['batchSize'], shuffle = True, num_workers = 0)

        data_val = SliceData(config['dataroot'], mode = 'validation')
        self.val_data = DataLoader(data_val, batch_size= config['batchSize'], shuffle=False, num_workers = 0)

        test_data = SliceData(config['dataroot'], mode = 'test')
        self.test_loader = DataLoader(test_data, batch_size = config['batchSize'], shuffle = True, num_workers =0)
    ...
    def train(self):
        ...
        for epoch in range(self.config['epoch'], self.config['n_epochs']):
            for i, batch in enumerate(self.dataloader):
                ...
                attn_A_index = batch['DX']
                attn_A,_ = self.aux_data.get_attn_map(attn_A_index, 0)
                if self.config['bidirect']:   # C dir
                    ...
                    if self.config['regist']:    # NC+R
                        ...
                        self.optimizer_G.zero_grad()
                        #### regist sys loss
                        concat_attn_A = attn_A
                        # print('-----conat a type------', concat_attn_A.dtype)
                        fake_B = self.netG_A2B(real_A*(1. + concat_attn_A))  #outG
                        
                        
                        o_crop = fake_B[:,:,50:110,60:120]  #o , center cropping
                        t_crop = real_B[:,:,50:110,60:120] #t
                        ...
                        #changed
                        pred_fake, tmp_attn = self.netD_B(o_crop, fake_B)
                        pred_real, _= self.netD_B(t_crop, real_B)
                        real_label = torch.ones(pred_real.shape).cuda()
                        # print('=============types====', pred_fake.device)
                        # print('========real=======', real_label.device)
                        adv_loss = self.config['Adv_lamda']*self.MSE_loss(pred_fake, real_label)
                        ...
                        with torch.no_grad():
                            concat_attn_A = attn_A
                            fake_B = self.netG_A2B(real_A*(1. + concat_attn_A))
                            o_crop = fake_B[:,:,50:110,60:120]
                            t_crop = real_B[:,:,50:110,60:120]
                        #fake
                        pred_fake,_ = self.netD_B(o_crop.detach(), fake_B.detach())
                        fake_label = torch.zeros(pred_fake.shape).cuda()
                        ...
                        #combined loss
                        loss_D_B = (loss_D_fake + loss_D_real)*0.5*self.config['Adv_lamda']
                        ...
                        loss_D_B.backward()
                        self.optimizer_D_B.step()
                        self.aux_data.update_attn_map(attn_A_index, tmp_attn.detach(), True)
    ...
    #         # Save models checkpoints
            ...
            torch.save(self.netG_A2B.state_dict(), self.config['save_root'] + 'netG_A2B_batch_1_ssim.pth')
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(self.val_data):
                    ...
                    # print(f" real a {r.shape} reaeal_Al b {real_B.shape} fake b {fake_B.shape}")
                    mae = self.MAE(fake_B,real_B)
                    psnr = PSNR_O(fake_B, real_B, data_range= real_B.max())
                    ssim = SSIM_O(fake_B, real_B, data_range = real_B.max())
                    PSNR_ +=psnr
                    SSIM_ +=ssim
                    MAE += mae
                    num += 1
                # print(f" real a {real_A.shape} real b {real_B.shape} fake b {fake_B.shape}")
                # print('======num value===', num)
                # print('------mae-----', MAE)
                

                # average_loss = MAE/num
                average_loss = SSIM_/num
                # print('=======SSIM=======', SSIM)
                print(f"PSNR: {PSNR_/num} SSIM :{SSIM_/num}  Val MAE:{MAE/num}")
            #     self.logger.log({'loss_D_B': loss_D_B,'SR_loss':SR_loss,'VAl_MEA': MAE})
            #     # print('---validation done----')
                
                if average_loss < best_loss:
                    best_loss = average_loss
                    best_weights = self.netG_A2B.state_dict()

            torch.save(best_weights, self.config['save_root'] + 'best_model_weights_batch_1_ssim.pth')
                    

                # print ('Val MAE:',/num)
                # print(f"PSNR: {PSNR_/num} SSIM :{SSIM_/num}  Val MAE:{MEA/num}")
    ...
    def test(self):
        ...
        # print(device)
        self.netG_A2B.load_state_dict(torch.load(self.config['save_root'] + 'best_model_weights_batch_1_ssim.pth', map_location='cpu'))
        ...
        with torch.no_grad():
                ...
                for i, batch in enumerate(self.test_loader):
                    ...
                    num += 1
                print ('MAE:',MAE/num)
                print ('PSNR:',PSNR_/num)
                print ('SSIM:',SSIM_/num)
    ...
    def MAE(self,fake,real):
        val = np.mean(np.abs(real-fake))/2
        return val
