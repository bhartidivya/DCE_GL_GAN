...
import argparse
import os
from trainer import Cyc_Trainer_nw,Nice_Trainer,P2p_Trainer,Munit_Trainer,Unit_Trainer
import yaml
...
def main():
    ...
    parser.add_argument('--config', type=str, default='Yaml/CycleGan_nw.yaml', help='Path to the config file.')
    opts = parser.parse_args()
    config = get_config(opts.config)
    
    if config['name'] == 'CycleGan':
        trainer = Cyc_Trainer_nw(config)
